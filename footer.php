



<footer class="footer">

  <img class="footerLogo" src="<?php echo get_template_directory_uri(); ?>/img/footer/logo-blanco-1.png" alt="CBbC GROUP">

  <a class="footerContacto" href="<?php echo site_url('/contacto'); ?>" target="_blank">Contacto</a>

  <div class="socialContainer">
    <a class="socialItem facebookFooter" href="https://es-es.facebook.com/CalaBassaBeachClub/" target="_blank">
      <svg aria-hidden="true" viewBox="0 0 512 512"><path fill="currentColor" d="M504 256C504 119 393 8 256 8S8 119 8 256c0 123.78 90.69 226.38 209.25 245V327.69h-63V256h63v-54.64c0-62.15 37-96.48 93.67-96.48 27.14 0 55.52 4.84 55.52 4.84v61h-31.28c-30.8 0-40.41 19.12-40.41 38.73V256h68.78l-11 71.69h-57.78V501C413.31 482.38 504 379.78 504 256z"></path></svg>
    </a>
    <a class="socialItem twitterFooter" href="https://twitter.com/cbbc_ibiza?lang=es" target="_blank">
      <svg aria-hidden="true" viewBox="0 0 400 400"><circle fill="currentColor" cx="200" cy="200" r="200"/><path style="fill:var(--color4)" d="M163.4,305.5c88.7,0,137.2-73.5,137.2-137.2c0-2.1,0-4.2-0.1-6.2c9.4-6.8,17.6-15.3,24.1-25c-8.6,3.8-17.9,6.4-27.7,7.6c10-6,17.6-15.4,21.2-26.7c-9.3,5.5-19.6,9.5-30.6,11.7c-8.8-9.4-21.3-15.2-35.2-15.2c-26.6,0-48.2,21.6-48.2,48.2c0,3.8,0.4,7.5,1.3,11c-40.1-2-75.6-21.2-99.4-50.4c-4.1,7.1-6.5,15.4-6.5,24.2c0,16.7,8.5,31.5,21.5,40.1c-7.9-0.2-15.3-2.4-21.8-6c0,0.2,0,0.4,0,0.6c0,23.4,16.6,42.8,38.7,47.3c-4,1.1-8.3,1.7-12.7,1.7c-3.1,0-6.1-0.3-9.1-0.9c6.1,19.2,23.9,33.1,45,33.5c-16.5,12.9-37.3,20.6-59.9,20.6c-3.9,0-7.7-0.2-11.5-0.7C110.8,297.5,136.2,305.5,163.4,305.5"/></svg>
    </a>
    <a class="socialItem instagramFooter" href="https://www.instagram.com/calabassabeachclub/" target="_blank">
      <svg aria-hidden="true" viewBox="0 0 512 512"><path d="M331.529,139.937H182.498c-23.8,0-43.14,19.34-43.14,43.14v149.008c0,23.8,19.339,43.167,43.14,43.167      h149.032c23.773,0,43.141-19.367,43.141-43.167V183.077C374.67,159.276,355.303,139.937,331.529,139.937L331.529,139.937z       M257,335.05c-42.705,0-77.467-34.763-77.467-77.468c0-42.705,34.763-77.467,77.467-77.467      c42.732,0,77.467,34.762,77.467,77.467C334.467,300.287,299.705,335.05,257,335.05L257,335.05z M336.97,196.133      c-10.119,0-18.333-8.211-18.333-18.33c0-10.095,8.214-18.333,18.333-18.333c10.091,0,18.333,8.238,18.333,18.333      C355.303,187.922,347.061,196.133,336.97,196.133L336.97,196.133z M336.97,196.133" fill="currentColor"/><path d="M257,212.865c-24.644,0-44.718,20.047-44.718,44.717c0,24.671,20.074,44.745,44.718,44.745      c24.671,0,44.745-20.074,44.745-44.745C301.745,232.938,281.671,212.865,257,212.865L257,212.865z M257,212.865" fill="currentColor"/><path d="M257,6.902c-138.07,0-250,111.93-250,250c0,138.07,111.93,250,250,250c138.07,0,250-111.93,250-250      C507,118.833,395.07,6.902,257,6.902L257,6.902z M407.392,332.085c0,41.861-34.027,75.889-75.862,75.889H182.498      c-41.834,0-75.889-34.027-75.889-75.889V183.077c0-41.831,34.055-75.886,75.889-75.886h149.032      c41.835,0,75.862,34.055,75.862,75.886V332.085z M407.392,332.085" fill="currentColor"/></svg>
    </a>
    <a class="socialItem pinterestFooter" href="https://www.pinterest.es/CBbCGroup/" target="_blank">
      <svg class="googleFooter pinterestFooter" fill="currentColor" version="1.0" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
      width="38.062px" height="38.061px" viewBox="0 0 38.062 38.061" enable-background="new 0 0 38.062 38.061" xml:space="preserve">
        <g id="XMLID_1678_">
          <path id="XMLID_1680_" fill="currentColor" d="M19.03,38.061L19.03,38.061C8.52,38.061,0,29.541,0,19.031l0,0C0,8.521,8.52,0,19.03,0
          l0,0c10.51,0,19.031,8.521,19.031,19.031l0,0C38.062,29.541,29.54,38.061,19.03,38.061z"/>
          <path id="XMLID_1679_" fill="#232323" d="M12.94,20.798c0.297,0.122,0.563,0.006,0.65-0.324c0.059-0.228,0.201-0.803,0.265-1.042
          c0.087-0.326,0.053-0.439-0.187-0.723c-0.522-0.617-0.856-1.416-0.856-2.546c0-3.281,2.455-6.218,6.392-6.218
          c3.486,0,5.401,2.13,5.401,4.976c0,3.743-1.657,6.902-4.116,6.902c-1.358,0-2.374-1.123-2.049-2.5
          c0.392-1.645,1.146-3.42,1.146-4.607c0-1.063-0.57-1.949-1.75-1.949c-1.389,0-2.504,1.437-2.504,3.36
          c0,1.225,0.415,2.055,0.415,2.055s-1.421,6.02-1.67,7.074c-0.497,2.1-0.075,4.674-0.039,4.934c0.021,0.154,0.219,0.19,0.309,0.074
          c0.128-0.167,1.782-2.209,2.343-4.248c0.16-0.578,0.914-3.568,0.914-3.568c0.451,0.86,1.77,1.617,3.171,1.617
          c4.174,0,7.004-3.804,7.004-8.897c0-3.851-3.26-7.438-8.218-7.438c-6.168,0-9.279,4.422-9.279,8.11
          C10.282,18.073,11.127,20.059,12.94,20.798z"/>
        </g>
      </svg>
    </a>
  </div>

  <ul class="footerMenu footerMenu1">
    <li class="footerMenuItem" onclick="alternateNavBar()"><a href="#">SITEMAP</a></li>
    <li class="footerMenuItem"><a href="<?php echo site_url('/ibiza'); ?>">Ibiza</a></li>
    <li class="footerMenuItem"><a href="<?php echo site_url('/andorra'); ?>">Andorra</a></li>
    <li class="footerMenuItem"><a href="<?php echo site_url('/contacto'); ?>">Grupos y eventos</a></li>
    <li class="footerMenuItem"><a href="<?php echo site_url('/contacto'); ?>">Restauración</a></li>
    <li class="footerMenuItem"><a href="<?php echo site_url('/contacto'); ?>">Reservas y contacto</a></li>

  </ul>
  <ul class="footerMenu footerMenu2">
    <li class="footerMenuItem"><a href="<?php echo site_url('/contacto'); ?>">Empleo</a></li>
    <li class="footerMenuItem"><a href="<?php echo site_url('/aviso-legal-y-confidencialidad'); ?>">Aviso legal & Privacidad</a></li>
    <li class="footerMenuItem"><a href="<?php echo site_url('/contacto'); ?>">Publicidad</a></li>
    <li class="footerMenuItem" onclick="alternateNewsLetter()"><a href="<?php echo site_url('/magazine'); ?>">Newsletter</a></li>
    <li class="footerMenuItem"><a href="<?php echo site_url('/home'); ?>">© Copyright 2019 CBbC Group</a></li>
  </ul>



<?php echo do_shortcode('[mc4wp_form id="1186"]') ?>


</footer>

<?php wp_footer(); ?>
</body>
</html>
